STATUS: I GANG
Iteration: 37 — 2026-09-26
Arbejdsgrene: `ceo/content-truth` (P1-21, målt + fix)
Næste handling: **P1-21 er færdig.** ❓ 2/❓ 3 er stadig ubesvarede, så målingen gik videre med de to overflader P1-20 anbefalede: `check --json` og Action-payloaden på de samme tal-spørgsmål. Den fandt **ét** fund, og det lå i den gratis vare, i et tal ingen server kan have sendt. P2-1 del B satte 2 MiB på indholdslæsningen, så én stor side ikke kunne trække watch-loopen ned, og det var rigtigt — men loftet var usynligt, og det tal, der blev lagt tilbage, var ikke en måling af siden. Målt med den rigtige CLI mod en 5 MiB-side uden `content-length`, nul kode ændret: `"contentLength": 2162237, "contentHash": null` — siden er 5 242 880 byte, og 2 162 237 var **hvor vores egen læser standsede**, før den afbrød strømmen. Den flyttede sig til 2 120 910 på den næste identiske kørsel, fordi den afhænger af, hvad socketzen nåede at levere. En CI-jobb der rapporterer sidestørrelse fra feltet publicerede altså et tal ingen server sendte, og et andet hver gang. Dertil: `tooLarge` blev skrevet i `content.js:96,101` og læst af **ingen** (`rg` bekræfter kun de to skrivesteder), og checkerens egen forklaring ("Page is 5242880 bytes — over the 2097152-byte content-check limit") blev beregnet og smidt væk, så menneske-fladen skrev **ingen Content-linje overhovedet** og `contentHash: null` var uadskilneligt fra en side, der virkelig ingen hash har. Rettelsen: `readContentState()` i `src/status.js` er den ene læsning af indholdstjekket — `measured` er sand kun når kroppen virkelig blev læst og hashet, `length` er det tal der beskriver siden (vores læsning, eller serverens **egen** `content-length`-erklæring; streaming-artefakten kommer aldrig ud som en sidestørrelse, men som `atLeast`), og `skipped` navngiver det ene bevidste spring. `content.js` skelner derfor mellem de to veje: erklæret for stor beholder serverens tal, streamet for stor får `contentLength: null` + `atLeastBytes`. `check --json` får `contentChecked` + `contentSkipped` (additive) og `contentLength` er `null` når siden ikke blev læst — samme `null` som P1-20 gjorde for et umuligt statusnummer, altså samme ærlige "ukendt" i stedet for et plausibelt tal. Menneske-fladen skriver nu `⏭️  Content: not read — page over the 2,097,152-byte content-check limit (read 2,162,237 bytes before stopping)`. **Action-payloaden er målt og fundet uberørt:** den læser hverken `contentLength` eller `contentHash` (tabellen har ingen content-kolonne), så fejlen kunne ikke nå den — den blev slet ikke målt til at være ren, den blev slet ikke nået. 4 nye tests (2 i `test/test.js` på checkeren, 1 på ejeren med 6 skæve recordformer, 1 end-to-end på begge flader) + 5 målte mutationer (artefakten som sidestørrelse → 2 fejl; skip-værdien væk → 2; checkeren sætter `contentLength: bytes` igen → 2; menneske-linjen væk → 1; `contentLimit` væk fra faktummet → 3) → **298/298**; `npm run audit` 0/0, `node --check`, `matrix --check`, `sh -n`/`bash -n` og diff-check grønne på Node 26.7.0. Én eksisterende test er ikke rørt. **Ingen exit-kode, ingen matrix-række, ingen ny claim, ingen deploy-note nødvendig.** Næste iteration: ❓ 2/❓ 3 hvis besvaret, ellers **P1-22**, som er målt i samme kørsel men ikke rettet (øverst i køen, se afsnittet).
...
### P1-22 — MÅLT 2026-09-26, IKKE RIGTET — `sslExpiringSoon: false` på en side hvor intet certifikat nogensinde blev set

**Begrundelse:** fundet i P1-21's måling af den samme overflad, og det er samme fejltype som P1-14 (to terminal-lister, der sagde `·` om både \"aldrig målt\" og \"verdikt ulæseligt\") og P1-17 (en payload, der påstod en overgang DeskUptime aldrig så).

**Målt:** `src/engine.js:79` læser SSL **kun** når `reachable && url.startsWith('https://')`. `check --json` skriver alligevel fire felter på ethvert resultat, også der hvor intet certifikat blev set:

| Site | `sslDaysRemaining` | `sslExpired` | `sslExpiringSoon` | `sslError` |
|---|---|---|---|---|
| `http://` (inget certifikat findes) | `null` | `false` | **`false`** | `null` |
| `https://` der svarer 500 (DOWN) | `null` | `false` | **`false`** | `null` |
| `https://` der ikke svarer | `null` | `false` | **`false`** | `null` |

`sslExpiringSoon: false` læses som \"certifikatet blev målt, og det er ikke snart udløbet\". Det er en påstand om en måling, der aldrig fandt sted — præcis det, `readSslState()`s egen `unreadable`-flag blev født til at forhindre på den anden side af samme funktion.

**Dertil:** reglen \"et certifikat er kun relevant for https\" har **to ejere** uden for `readSslState()`: `engine.js:79` (spring checken over) og `action.yml:130` (`x.url.startsWith("https")` i SSL-tælleren). De er enige i dag, men ingen af dem er ejeren, og `readSslState()` siger intet om hvorvidt et certifikat overhovedet blev set.

**Hvorfor den ikke blev rettet samme iteration:** den kræver et nyt begreb i `readSslState()` (`measured: false` for en url uden certifikat) og dermed en beslutning om, hvad `sslExpiringSoon` skal være for disse rækker — `null` mod `false` er en **kontraktaendring** i et felt, et Action-job og `jq`-brugere i dag læser som en boolean. Den fortjener sin egen iteration med målte mutationer, ikke en halv time inde i en anden.

**Acceptkriterier (forslag):**

1. `readSslState()` kan sige \"no certificate to read\" skelnet fra \"read, and not expiring\". — Ejer: `src/status.js`.
2. `check --json` må ikke skrive et boolsk `sslExpiringSoon` for en url, hvor intet certifikat blev set. — Enten `null` (kontraktaendring, skal noteres) eller et additivt `sslChecked: false`.
3. `action.yml:130` spørger ejeren i stedet for at genkende `https` selv. — Én regel, én ejer.
4. Menneske-fladens `— SSL: N/A` og Action-tabellens `—` må ikke sige det samme som P1-14s `— not checked yet` gør i terminal-listerne.

**Filer (foreløbig):** `src/status.js`, `src/cli.js`, `action.yml`, `test/test.js`, `test/status.test.js`.
