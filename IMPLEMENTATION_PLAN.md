STATUS: I GANG
Iteration: 46 — 2026-09-26
Arbejdsgrene: `ceo/report-source-truth` (P1-30, målt + fix)
Næste handling: **P1-30 er færdig.** `deskuptime report` — den eneste flad der aldrig er målt med rigtig CLI, fordi den er Pro-gated — viste sig at have **to** fejl, begge i den betalte vare. Målt først med rigtig `check` + `watch --once` mod tre lokale fixture-servere (én 200, én 500, én der 301er til en anden vært), Pro-stub i state-filen, og rigtig `report` + `report --json` + `status` + `watch --status` på både en rigtig state-fil og en håndskrevet, nul kode ændret.
...
Én række, to påstande om samme site: `Uptime (window)` kommer fra `history.json`, resten af rækken fra `state.json`, og rapporten læste dem uden at sammenligne dem. De to filer er skrevet af **samme** pass, så de kan uoverensstemme kun fordi de er to filer: historik-skrivningen er med vilje pakket i `try/catch`, så overvågningen fortsætter når den fejler (`watch.js:292-299`), og et bureau der flytter overvågning til en ny maskine kopierer den ene fil README nævner. Rettelsen: `windowSummary` får passens `lastChecked` og svarer med `passNotRecorded: true` i stedet for det stille `null`; cellen siger `— (last check missing from the history file)`, og `--json` kan gengive sætningen. **Ingen falsk alarm:** et pass *uden for* vinduet, et aldrig tjekket site og et *fremtidigt* tidspunkt beholder `— (no pass in the last 30 d)` — den nye sætning må ikke kunne dukke op på en almindelig rapport.

**Fund 2 — `--json` førte de to sidste rå strenge fra state-filen**, mens Markdown i samme dokument viste `—`:
...
Ny `readPassTime()` i `src/status.js` er den ene læsning af et registreret tidspunkt, `readStatusCode`'s søskende: ulæseligt bliver `null`, læsbart canonicaliseres til ISO, så JSON og Markdown-cellen bærer samme øjeblik.

**En fejl i min egen rettelse, målt og rettet:** canonicaliseringen alene fik opsummeringslinjen til at skrive `1 not checked` om et site hvis egen række sagde, at tidspunktet var ulæseligt — `siteBuckets` testede `!site.lastChecked`, og `null` dækker både "aldrig tjekket" og "ulæseligt". Det er præcis den sammenfaldning P1-14 blev skrevet for at forhende, så "en pass blev overhovedet registreret" er nu sit **eget felt** (`passRecorded`), og `unknownNote()` tager den kendsgerning som input i stedet for at gætte på en strengs sandhed.

4 nye tests → **329/329**; audit 0/0; `node --check`, `matrix --check` og `git diff --check` grønne på Node 26.7.0. Fire mutationer målt, alle døde: rå streng i `--json` (2 fejl), cellen uden navnet (1), `emptyWindow` inverteret (5), `never-checked` læser den læsbare tid igen (1). To **eksisterende** tests lå på den gamle sætning og blev rettet med begrundelse (fjerde og femte gang en lås følger en målt rettelse) — noteret i rækkefølgen nederst. Næste opgave: **P1-31**.
...
- Root: `npm test` (329 tests, 329 passed på Node 26.7.0 efter P1-30: 325 + 4 nye tests om rapportens to kilder og dens tidsstempler; tællet stiger med hver målte iteration, så læs tallet herfra `npm test` selv). **Bemærk:** på en maskine med kun Node 22 fejler de 7 installtests + action-testen, fordi `install.sh` og `action.yml` korrekt kræver Node 24+; brug den installerede `PATH`-node (26.7.0) — på Mads' maskine `/opt/homebrew/opt/node@26/bin/node` — ellers er gate ikke grøn af miljøårsager.
...
### P1-31 — NÆSTE — et fremtidigt tidspunkt rapporteres som "tjekket i dag", og den negative alder, `checkAgeMs` siger den leverer, kasseres
