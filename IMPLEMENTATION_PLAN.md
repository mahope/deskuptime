...
STATUS: I GANG
Iteration: 23 — 2026-09-26
Arbejdsgrene: `ceo/report-stale` (P1-6)
Næste handling: **P1-6 er færdig** — kundenapporten viste et site checket **41 dage siden** som `UP (200) · 100%` og talte det med i `2 up · 0 down`, helt identisk med et site checket 8 minutter siden. Rapporten genkører intet (read-only, ingen request), så "op" var en påstand uden alder, og en død watch-loop så ud som et sundt site i det præcis dokument et bureau videresender til sin kunde. Målingen (før kode) er bevaret i iterationsloggen; `Uptime (window)` vidste allerede svaret med `— (no pass in the last 30 d)`, mens Status-kolonnen og resumelinjen sagde det modsatte. Ny `STALE_AFTER_DAYS = 2` + `isCheckStale()` i `src/status.js` (ét sted, sammen med `SSL_WARN_DAYS`): et site uden pass i 2 dage skriver `UP (200) ⚠️ stale — last check 41 d ago`, tælles i resumelinjen (`1 stale (no check in the last 2 d)`) og **navnes på en egen linje** med URL og alder; `summary.up` tæller kun up-to-date sites, mens `summary.down` tæller *alle* sites, for et nedet site skal en kunde stadig se. `--json` får `stale`, `ageDays` og `summary.stale`, og `summary.up`s ændrede betydning er dokumentet i `docs/agency-report.md` §4. 4 nye tests → **198/198**. Næste iteration: ❓ 2/❓ 3 hvis besvaret, ellers ny research-iteration. P1-1 AC3 (sitekilder) er `BLOCKED` på ❓ 8.
