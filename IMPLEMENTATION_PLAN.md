STATUS: I GANG
Iteration: 42 — 2026-09-26
Arbejdsgrene: `ceo/answered-by-another-host` (P1-26, research + målt + fix)
Næste handling: **den nye overflade P1-25 bad om er fundet, og fundet er den værste løgn i hele køen.** Ikke en ny kommando — den fælles antagelse under alle seks: at et `200` fortjener ordet UP. `ping.js` målte `response.url` på hvert check siden P0-3, `engine.js:69` skrev den i `result.finalUrl`, og dér døde den. Ingen flade kunne læse den, så **"UP" var en påstand om en URL, ingen havde bedt om.** Målt med rigtig CLI mod et site der 301er til en anden vært og svarer 200 dér:

```
check   ->  ✅ …  Status: 200 — UP          exit 0        (parket domæne / kapret / tastefejl)
headers ->  301 → …/lander   Final: …/lander (200) — redirected     ← den ene sande flade
```

Det er en kunde-domæne der udløber og bliver parkeret, et hijacket domæne der peger på en phishing-side, eller en tastefejl der lander på "mente du"-siden — alle tre svarer 200, og et bureau's kundenrapport siger 100 % uptime i en måned. `readRedirectTarget()` i `src/status.js` er nu den ene ejer; `check` fortæller det i begge flader, `--json` får `finalUrl` + `offHostRedirect`. En redirect er bevidst **ikke** DOWN — `www → apex` er den mest almindelige redirect på nettet. 3 nye tests → **317/317**; audit 0/0; **ingen eksisterende test rettet**. Næste opgave: **P1-27** — samme kendsgerning på de fire betalte flader (`watch`, `status`, `report`, webhook), hvor den endnu ikke kan ses.
...
### P1-27 — NÆSTE — Den samme kendsgerning på de betalte flader (`ceo/off-host-paid`)

**Begrundelse:** P1-26 lod kendsgerningen nå `check` — den gratis flade, der afgør exit-koden. **De fire flader hvor den *mest* gør skade, kan den stadig ikke se**, og alle fire er betalte eller bureau-brugte. Målt i P1-26's kørsel, uændret af rettelsen: `watch --once` skrev `baseline recorded: UP (200)`, `watch --status`/`status` skrev `✅ up … (200)`, `report` skrev `UP (200) | 100 %`, og webhook-payloaden (Pro-kanalen) bar den samme `type: "up"` uden at kende `finalUrl`. Ejeren findes allerede, så opgaven er at lade fladerne **spørge** frem for at eje — inkl. at `watch` gemmer `finalUrl` i state, så `report` og `status` kan vise det i næste pass.
...
2. Webhook-payloaden får ét additivt felt, så en Pro-kanal kan se at svaret kom fra en anden vært. `type`/`message`/`timestamp` uændret i det normale tilfælde.
3. Ét `watch`-event nævner skiftet, så en kunde med notifikationer hører om det på samme tid som de hører om DOWN.
4. Rækker, der *kun* har en sti-redirect på egen vært, er uændrede på alle fire flader (målt med samme to fixture-servere som P1-26).
5. `npm test` grøn med de nye tests, ingen eksisterende test rettet.
...
**Begrundelse (kun noteret, ikke målt endnu):** `src/checkers/ping.js:15` genkender "serveren svarede ikke på HEAD" som `{404, 405, 501}`. Mange WAF'er og bot-filtre svarer **403** på HEAD og 200 på GET, så et helt sundt site rapporteres DOWN med `errorType: "http_error"`. Det er den modsatte fejltype — en **falsk DOWN** i stedet for en falsk UP — og den er dyrere for et bureau end de andre, fordi den får kunden til at tro at sitet er nede. **Skal måles først** (andre 4xx-koder, fx 401/403/429, kan have samme årsag), og rettelsen skal *ikke* blive "kast 403 væk": 403 på HEAD *kan* være en ægte blokering, så kriteriet er at GET genprøves på de koder, der dokumenteret betyder "HEAD er ikke understøttet her", og at et 403 der overlever GET-forsøget stadig er DOWN.

**Acceptkriterier:**

1. Målt med rigtig CLI mod en lokal server der svarer 403 på HEAD og 200 på GET, før nogen kode ændret — output fra **alle** flader noteret.
2. Kun de koder, der faktisk er målt som "HEAD understøttes ikke", udvides; et 403 der overlever GET'en bliver DOWN.
3. Ny `const`-linje med en kildekommentar, der siger hvilken kode der er målt hvorfor, så næste iteration ikke gætter videre.
4. Gate grøn, ingen eksisterende test rettet.
