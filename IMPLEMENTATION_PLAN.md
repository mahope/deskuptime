...
status        ->  ✅ …/flyttet (200) ⚠️ answered by 127.0.0.1:64651 (asked 127.0.0.1:64652)
watch --status ->  samme linje
report        ->  | …/flyttet | UP (200) ⚠️ answered by 127.0.0.1:64651 | … | + navngiven linje + "1 answered by another host"
/gammel (egen vært) ->  ingen linje på nogen af dem
```

Fundet var ikke en ny løgn men en **kastet kendsgerning**: motoren måler `finalUrl` på hvert pass, og `runPass` skrev den aldrig videre, så intet efter passet *kunne* se den. Rettelsen følger rækken fra P1-13: `readRedirectTarget()` er stadig den **ene ejer** og fik et `label` (samme sætning som `note`, kort nok til en celle); `readEntry()` læser den for begge statuslister; rapporten spørger den selv; payloaden får præcis de to additive felter `check --json` allerede har. Hændelsen latches på den **svarende vært**, ikke på URL'en, så et parkeret domæne ikke sender en betalende kunde en notifikation hvert minut, mens et skift til en anden fremmed vært stadig høres. 4 nye tests → **321/321**; audit 0/0; **ingen eksisterende test rettet**. Næste opgave: **P1-28** — samme kendsgerning i `action.yml`'s step-summary.
