...
Arbejdsgrene: `ceo/status-line-truth` (P1-7)
Næste handling: **P1-7 er færdig** — ❓ 2 og ❓ 3 er stadig ubesvarede, så iterationen blev brugt til research, og målingen spurgte: *hvilke andre overflader viser de samme tre fakta om et overvåget site, og beslutter de dem selv?* Svaret var de to statuslister. P1-2b (SSL), P1-5 (tællere) og P1-6 (forældede data) rettede hver én flade, men `deskuptime watch --status` og URL-listen på `deskuptime status` afgør stadig selv, hvad de siger. Målt på én rigtig state-fil gav det tre fejl samlet: et pass fra **41 dage siden** stod som `✅ up` uden alder nogen steder (mens kundenapporten fra samme fil sagde `⚠️ stale — last check 41 d ago`); et certifikat med **9 dage** tilbage stod som `SSL 9d` i almindelig tekst, selv om `check`, `watch` og rapporten alle advarer i 14-dages vinduet; og **escape-sekvenser i `sslValidDays` nåede terminalen** (`^[[2J^[[5mOWNED-BY-STATE-FILE^[[0m`) i begge flader, fordi feltet blev sat sammen med `+` — den eneste printside, P2-1 del C ikke dækkede — mens en ubrugelig værdi stod som `SSL -2d`, hvor rapporten korrekt viser `—`. Ny `readEntry()` i `src/status.js` er nu den *ene* læsning af en state-entry: den bestemmer verdict, statuskode, certifikat-note og alder med de regler, `check`/`watch`/rapporten allerede brugte, og returnerer **kun kontrollerede tal og faste ord** — så et råt felt kan ikke skrives ud, selv hvis en kunde skriver en. `watch --status` markerer et forældet pass med alder og **navner de døde sites i en egen blok** med den kørsel, der genopbygger dem, fordi det er det eneste kommando der kan fortælle brugeren at *overvågningen* er stoppet. Exit-koder er bevidst uændrede (også for et DOWN-site: de var 0 i forvejen, så en exit-ændring ville bryde scripts uden at gøre noget ærligt). 15 nye tests i `test/statusline.test.js` → **213/213**. Næste iteration: ❓ 2/❓ 3 hvis besvaret, ellers ny research-iteration. P1-1 AC3 (sitekilder) er `BLOCKED` på ❓ 8.
