STATUS: I GANG
Iteration: 41 — 2026-09-26
Arbejdsgrene: `ceo/falsy-measured-facts` (P1-25, målt + fix)
Næste handling: **P1-25 er færdig**, og den er den første opgave i denne række, hvor målingen **ikke** bekræftede opgavens egen hypotese helt. P1-24 havde fundet tre `||` i samme mønster og antaget, at alle tre var fejl; målingen siger, at **kun det første var det.** En server der sender `X-Powered-By: ` tomt faldt gennem `h['x-powered-by'] || null`, så den **navngivne advarsel** `X-Powered-By exposed` forsvandt helt — værktøjet sagde til et bureau at kundens site ikke afslører sin stack, mens serveren sendte headeren. Det er rettet: `headerState()` i `src/status.js` er den ene klassificerer (P1-24's `readSecurityHeaders` spørger den nu), `readDisclosure()` er den nye ejer af `server`/`poweredBy`, checkeren skriver `?? null`, og `cli.js` læser ejeren én gang i begge flader med et additivt `disclosureEmpty` i JSON.
...
De to andre steder (`content.js:121`, `watch.js:156`) blev målt på en håndskrevet `state.json` gennem rigtig `watch --once` og viste sig **ikke** at være fejl: `lastHash: ""` opfører sig præcis som `null` — ingen `changed`-påstand, ingen falsk alarm — og samme pass reparerer filen til en rigtig 64-hex hash. Asymmetrien mod headerne har en grund: en tom `X-Powered-By` er noget en rigtig server sender på ledningen, en tom hash er noget DeskUptime aldrig har skrevet. De steder er urørte og låst af en test, så næste iteration ikke "fikser" dem. **Dette er hele pointen med at måle først: to af tre kandidater var ikke fejl, og det ville ikke være set uden målingen.** 5 nye tests + 6 målte mutationer (alle døde) → **314/314**; audit 0/0; **ingen eksisterende test rettet**. Næste opgave: research-iteration — se `P1-25`s "Næste opgave".
...
- Root: `npm test` (314 tests, 314 passed på Node 26.7.0 efter P1-25: 309 + 5 nye tests om de to disclosure-headere; tællet stiger med hver målte iteration, så læs tallet herfra `npm test` selv). **Bemærk:** på en maskine med kun Node 22 fejler de 7 installtests + action-testen, fordi `install.sh` og `action.yml` korrekt kræver Node 24+; brug den installerede `PATH`-node (26.7.0) — på Mads' maskine `/opt/homebrew/opt/node@26/bin/node` — ellers er gate ikke grøn af miljøårsager.
...
**Næste opgave:** køen er tom på dette spor (P1-13…P1-25 er lukket, og ❓ 2/❓ 3 er stadig ubesvarede). Næste iteration bør være en **research-iteration**: de målte fejl er nu lukket i `headers`/`check`/`report`/webhook, så den bør lede efter en *ny* overflade med de samme tal-spørgsmål — ikke flere `||`.
