STATUS: I GANG
Iteration: 45 — 2026-09-26
Arbejdsgrene: `ceo/head-403-fallback` (P1-29, målt + fix)
Næste handling: **P1-29 er færdig.** Et site bag en WAF der svarer 403 på `HEAD` blev rapporteret som **DOWN på alle flader**. Målt før rettelsen med rigtig CLI mod en lokal server der svarer 403 på HEAD og 200 på GET, nul kode ændret:

```
check        exit=2   ❌ …  Status: 403 — DOWN   ⚠️ Error: HTTP 403
check --json exit=2   healthy=false statusCode=403 errorType=http_error
watch --once exit=2   • … baseline recorded: DOWN — HTTP 403      ← gemt som DOWN
status       ❌ … (403)
watch --status 🚨 down … (403)                                    ← ordet "down" til kunden
```

Der blev **intet GET-forsøg** — de eksisterende koder `{404, 405, 501}` dækker ikke 403. Efter rettelsen mod de **samme** fixtures:
...
HEAD 401 / GET 200  ->  ❌ 401 — DOWN, ingen ekstra request    (uændret)
HEAD 429 / GET 200  ->  ❌ 429 — DOWN, ingen ekstra request    (uændret)
```

Rettelsen er **én konstant** i `src/checkers/ping.js`: `403` tilføjes til `HEAD_UNSUPPORTED`, så vejen går gennem den samme `--timeout`-budget som P0-13's 404/405/501-genprøv. Ingen ny kodevej, intet nyt felt. **403 er ikke kastet væk:** kun GET-svaret bruges, så et 403 der svarer 403 på GET'en stadig er DOWN med sit oprindelige statusnummer — og det er den rigtige retning, fordi 401 og 429 *er* egenskaber ved hele ressourcen, ikke ved metoden, så en genprøv der kan ændre intet kun koster en request (til en rate limiter endda: en ekstra request fra en klient der lige har fået at vide den skal sænke farten).

En read-only stikprøve af 12 offentlige sites bekræfter begge halvdele: `www.netflix.com` svarer **405 på HEAD og 200 på GET** (mekanismen er virkelig, ikke kun en fixture), og `stackoverflow.com` svarer **403 på begge** (et 403 der overlever GET'en er en ægte blokering). Ingen af de 12 svarer 403 på HEAD og 200 på GET, så den 403-halvdel hviler på WAF'ernes dokumenterede opførsel — og netop derfor er genprøven den sikre rettelse. 3 nye tests → **325/325**; audit 0/0; `node --check` og `git diff --check` grønne på Node 26.7.0; **ingen eksisterende test rettet**. Mutationstest: uden rettelsen fejler 2 af de 3 nye tests. Næste opgave: **P1-30** — `deskuptime report` er den eneste flad, der **aldrig** er målt med rigtig CLI, fordi den er Pro-gated; den er det dokument et bureau sender videre til en kunde.
...
- Root: `npm test` (325 tests, 325 passed på Node 26.7.0 efter P1-29: 322 + 3 nye tests om HEAD-genprøven ved 403/401/429; tællet stiger med hver målte iteration, så læs tallet herfra `npm test` selv). **Bemærk:** på en maskine med kun Node 22 fejler de 7 installtests + action-testen, fordi `install.sh` og `action.yml` korrekt kræver Node 24+; brug den installerede `PATH`-node (26.7.0) — på Mads' maskine `/opt/homebrew/opt/node@26/bin/node` — ellers er gate ikke grøn af miljøårsager.
...
### P1-30 — NÆSTE — mål den betalte rapport mod de samme tal-spørgsmål

**Begrundelse:** ❓ 2 og ❓ 3 er stadig ubesvarede, og `deskuptime report` er den eneste flad der **aldrig** er målt med rigtig CLI i dette repo, fordi den er Pro-gated. Alle de seneste fund (P1-26 → P1-29) var fund i de flader der *kan* måles, så det er et reelt blindpunkt: rapporten er den dokument et bureau sender til en kunde, og dens tal er de tal kunden husker. P1-5, P1-6 og P1-20 rettede den, men kun ud fra unit-tests, aldrig mod en rigtig kørsel.

**Fremgangsmåde:** kør `check` + `watch --once` mod lokale fixture-servere (samme opskrift som P1-29: asynkront `execFile`, fixture-serverne må ikke køre i samme proces som en synkron spawn) med en **licens-stub** som svarer `ok: true` på `validate`, så `report` åbner. Notér alle rækker, opsummeringslinjen og `--json`-felterne før nogen kode ændret.

**Acceptkriterier:**

1. Rigtig `report` + `report --json` kørt mod fixture-servere med Pro-stub, output fra alle rækker noteret før ændring.
2. Ethvert fund får en egen målt fejl, ikke en samlet " rapporten er forældet".
3. Hvis intet findes, skrives det i planen som et målt resultat med tal — ikke som "ser rigtigt ud".
