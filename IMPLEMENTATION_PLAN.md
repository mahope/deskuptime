...
STATUS: I GANG
Iteration: 16 — 2026-09-26
Arbejdsgrene: `ceo/body-cap-license-retry` (P2-1 del B)
Næste handling: **P2-1 del B er færdig**: indholdstjekket er nu begrænset i både størrelse og tid — `checkContentChange` læser højst `MAX_CONTENT_BYTES` (2 MiB) og abort-timeren er **først** ryddet efter body-readet, så en side der sender headere og så går i stå ikke kan hænge watch-loopen; et for stort svar giver *intet indholdssignal* (`fetched:false, tooLarge:true`), aldrig en falsk DOWN. Licenskald har én afgrænset genprøvning (`LICENSE_ATTEMPTS = 2`, 400 ms), som aldrig spørger om et *verdikt* igen og respekterer et langt `Retry-After` ved **ikke** at genprøve. 13 nye tests + 6 mutationstests. Næste iteration: **P1-2 del C** (30-dages historik + planlagt rapport — kræver ❓ 3) eller en ny research-iteration. P1-1 AC3 (sitekilder) er `BLOCKED` på ❓ 8.
