STATUS: I GANG
Iteration: 36 — 2026-09-26
Arbejdsgrene: `ceo/report-json-truth` (P1-20, målt + fix)
Næste handling: **P1-20 er færdig.** ❓ 2/❓ 3 er stadig ubesvarede, så målingen gik videre med den sidste u målte overflad, P1-19's egen anbefaling: `report --json` og de to terminal-lister på de samme tal-spørgsmål. Den fandt **to** fejl, begge i den betalte vare, og den ene lå i selve den overflad, en kunde og et bureau bruger hver sin vej. **Fund 1 — et tal uden for 100-599 blev trykt som et statusnummer.** `readEntry()` (`status.js:298`) og `buildReport()` (`report.js:184`) testede hver især `Number.isInteger` og ingenting mere, så en håndskrevet eller fra en backup gendannet `state.json` fik **alle fire** flader til at skrive et tal, ingen server kan have sendt: `| https://kode-1.dk/ | UP (-1) | 100% (2 checks) |` i kundedokumentet, `"statusCode": 9999` i `--json`, `· ✅ https://kode-1.dk/ (-1)` i `status` og `✅ up https://kode-2.dk/ (9999)` i `watch --status`. `readEntry()`s egen docstring lover, at hver værdi er "a fixed word or a checked number … so an unusable field can only become `null`, never a claim the caller did not check" — statusnummeret var det ene felt, løftet ikke dækkede, fordi det var skrevet to steder og tjekket med det svagere af de to regler. Bemærk hvad den eksisterende test ikke fangede: den dækkede `"200"`, `20.5`, `null`, `{}`, `[200]`, `true` og `NaN` — altså alle måder et *ikke-tal* kan se ud på — men ikke **-1**, fordi det jo er et tal. Rettelsen: `readStatusCode()` i `src/status.js` er den ene ejer, begge kaldssteder spørger den, og uden for 100-599 bliver `null` (samme `—` som hver søskendecelle) frem for at blive klemt ind i et plausibelt kode, hvilket ville være en anden løgn. **Fund 2 — kundelinjens partition fandtes ikke i JSON.** `buildReport()` beregnede `siteBuckets(sites)`, brugte `neverChecked` og kasserede resten, mens `renderReportMarkdown()` beregnede den **igen** fra `sites`; `summary` eksporterede i stedet de fire overlappende tal. Målt på én rigtig state-fil med syv sites (én up, én down, én stale, én aldrig tjekket, én med ulæseligt verdict, to med ødelagt statusnummer) var linjen en partition der lod sig lægge sammen, og JSON'en var det ikke: `**7 site(s) · 3 up · 1 down · 1 not checked · 1 status unknown · … · 1 stale**` mod `"sites": 7, "up": 3, "down": 1, "unknown": 2, "stale": 1` → 3+1+2 = **6 af 7**. Den partition, rapportens egen fodnote definerer, var ingen steder i JSON, så en CI-konsument eller et bureaus eget system **kunne ikke gengive den linje, kunden fik**. Rettelsen: partitionen følger med i rapporten som `partition`, linjen læser den i stedet for at beregne den, og `summary`s tolv nøgler er uændrede — de er kontraktet en eksisterende konsument allerede læser, så intet går i stykker. 4 nye tests (2 i `test/report.test.js`, 2 i `test/statusline.test.js`) + 5 målte mutationer (svagere regel → 3 fejl; rapporten læser `lastStatus` selv igen → 2; `partition` fjernet → 2; linjen genberegner i stedet for at læse → 1; out-of-range klemt til 100/599 → 2) → **294/294**; `npm run audit` 0/0, `node --check`, `matrix --check`, `sh -n`/`bash -n`, YAML-parse og diff-check grønne på Node 26.7.0. Én eksisterende test måtte rettes: `test/report.test.js` låser rapportens feltliste præcist, så `partition` er en beslutning og ikke en sidevirkning — den er derfor skrevet ind med sin egen nøgleliste. **Ingen exit-kode, ingen matrix-række, ingen ny claim, ingen deploy-note nødvendig.** Næste iteration: ❓ 2/❓ 3 hvis besvaret, ellers en ny research-iteration — `check --json` og Action-payload'en er endnu ikke målt på de samme tal-spørgsmål, og `action.yml` har kun været sweepet for fjendtlig tekst.
