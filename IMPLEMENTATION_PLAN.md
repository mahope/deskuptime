STATUS: I GANG
Iteration: 40 — 2026-09-26
Arbejdsgrene: `ceo/headers-empty-value` (P1-24, målt + fix) — merged til `main` som `f4efdfd`
Næste handling: **P1-24 er færdig.** ❓ 2/❓ 3 er stadig ubesvarede, så målingen gik videre med det fund P1-24 selv gjorde i sin egen rettelse: **`||` brugt som klassificering af en målt kendsgerning er en mønsterfejl, ikke et isoleret fejlpunkt.** P1-24 rettede den ene forekomst på den sikkerhedsvurdering, et bureau viser en kunde; en grep i samme kode fandt tre tilbage i samme form, hvoraf to er på *sikkerhedsfelter* (`server` og `x-powered-by` i `checkers/headers.js:134-135`) og to er på *content-hash-kæden* (`content.js:121`, `watch.js:156`) — altså to steder hvor et ukendt hash kunne ligne et målt resultat.

```
x-frame-options:   ->  ⬜ missing: x-frame-options     <- serveren SENDTE den (før)
                      ⚠️  sent with no value: x-frame-options   (nu)
   --json          ->  "x-frame-options": null         (før)   ->   ""  +  securityEmpty: ["x-frame-options"]  (nu)
```

Rettelsen: `readSecurityHeaders()` i `src/status.js` er den ene ejer af de tre tilstande (sendt / sendt-tom / ikke-sendt) ved siden af `readHttpsState`; `checkers/headers.js` skriver `?? null` i stedet for `|| null`; `cli.js` spørger ejeren **én** gang og læser den i både menneskefladen og `--json` (nyt additivt felt `securityEmpty`). Fem nøgler, `⬜ missing:`-linjer for en normal header, exit-kode og øvrige felter uændrede. 4 nye tests + 5 målte mutationer (2 / 2 / 2 / 1 / 2 fejl) → **309/309**; audit 0/0. **Næste opgave: P1-25**, som er bevidst formuleret som *en måling først*: to af de tre `||` er ubetydelige, og en opgave der hævder en fejl uden at måle den er den fejltype denne kø er født med.
...
### P1-25 — ÅBEN — Mål de tre sidste `||`, der stadig kaster en målt kendsgerning (`ceo/falsy-measured-facts`)

**Begrundelse:** P1-24 viste, at `h[name] || null` er en *mønsterfejl*, ikke et isoleret fejlpunkt — den er skrevet tre steder til i samme kode. Denne opgave er **en måling først**, fordi to af de tre er ubetydelige, og fordi en fjerde sted (`watch.js`) ikke har noget med headere at gøre: en opgave der hævder en fejl uden at have målt den, er den fejltype denne kø er født med.

**Kandidatsteder fundet ved grep i P1-24 (nul kode ændret, ingen af dem må læses som en påstand om en fejl):**

1. `src/checkers/headers.js:134-135` — `server: h['server'] || null, poweredBy: h['x-powered-by'] || null`. Samme form som P1-24, på to felter der **er** sikkerhedsfund bureauer viser en kunde: `X-Powered-By exposed` er en reel advarsel, så en server der sender den **tom** bør måske have samme tredje linje som de fem headere. Om den skal have den, afgøres af målingen.
2. `src/checkers/content.js:121` — `previousHash: previousHash || null`. Et gemt hash, der er den tomme streng, bliver `null` og dermed "vi vidste ikke, hvad der stod før", altså et **ukendt** der ligner et målt resultat.
3. `src/watch.js:156` — `contentHash: entry.lastHash || null`. Samme fejltype ét lag længere inde i overvågningsloops state-fil, som er håndskrevet og genskabt fra backup.

**Acceptkriterier:**

1. Målt, før der rettes. — **`X-Powered-By: ` (tom) og `lastHash: ""` i en håndskrevet `state.json` skal køres gennem rigtig CLI, med output fra begge flader noteret, før nogen linje ændres.**
2. En kendsgerning må ikke blive `null` uden at nogen siger det. — **Hvis målingen bekræfter at et tomt `X-Powered-By` rapporteres som "ikke eksponeret", skal det få sin egen linje ved siden af de fem headere, på samme måde som P1-24.**
3. Ét sted afgør klassificeringen. — **Hvis der rettes, hører beslutningen hjemme i `src/status.js` ved siden af `readSecurityHeaders`/`readHttpsState`, og både `cli.js` og JSON'en læser den.**
4. Rett ikke sted 2 og 3 uden egen måling. — **`content.js` og `watch.js` får højst en note, indtil en måling viser at et tomt hash gør noget forkert; et ukendt der ligner et målt resultat skal måles, ikke formodes.**
5. Gate uændret. — **`npm test` grøn, `npm run audit` 0/0, og de strukturelle låse fra P1-16, P1-21, P1-22, P1-23 og P1-24 er urørte.**

**Filer:** `src/status.js` (kun hvis målingen bekræfter), `src/checkers/headers.js`, `src/cli.js`, `src/checkers/content.js` og `src/watch.js` (kun hvis målingen bekræfter), `test/status.test.js`.
