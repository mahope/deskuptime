...
STATUS: I GANG
Iteration: 22 — 2026-09-26
Arbejdsgrene: `ceo/report-counter-clamp` (P1-5)
Næste handling: **P1-2 del D er lukket som P1-2b, og P1-5 er færdig** — kunderapporten kunne hævde *250 % uptime* og *−2 failed*. Målingen fandt den samme familie af fejl som de to foregående iterationer (ugyldigt SSL-tal, `failures > checks` i historikken), men i den sidste åbne sti: `src/report.js` gættede `NaN` væk med `intOrZero()`, men **klemmede aldrig `checksUp` til `checks`**. `history.js` klemmer sine dagsbucketter med en kommentar om præcis denne risiko; state-filen gjorde ikke, så en håndskrevet, gendannet eller halvskrevet `state.json` skrev `250 % (2 checks)` og `**… 6 checks · -2 failed**` i det dokument et bureau videresender til sin kunde. Ny `counters()` i `src/report.js` er nu det ene sted, hvor tællerne læses (forfatter, rapport og procentandel kan ikke komme uden om det), `recordPass()` læser gennem den og skriver derfor de **reparerede** tal tilbage på disk, så næste pass helbreder filen i stedet for at bevare fejlen. `uptimePercent` kan ikke overstige 100, `failures` ikke være negativ, hverken i Markdown, JSON eller resumelinjen. 3 nye tests → **194/194**. Næste iteration: ❓ 2/❓ 3 hvis besvaret, ellers ny research-iteration. P1-1 AC3 (sitekilder) er `BLOCKED` på ❓ 8.
