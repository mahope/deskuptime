...
STATUS: I GANG
Iteration: 33 — 2026-09-26
Arbejdsgrene: `ceo/payload-truth` (P1-17, mergeet til `main` som `536bf5a`, begge grene pushet 2026-09-26)
Næste handling: **P1-17 er færdig.** ❓ 2 og ❓ 3 var stadig ubesvarede, så målingen gik videre på den overflad P1-16 pegede på: `watch`s **webhook-payload** — den Pro-kanal der er *solgt*, og hidtil kun sweepet for fjendtlig tekst, aldrig målt på "er dette en måling vi har". Den viste sig at være det eneste sted i hele produktet, der **overhovedet oplyser et tidspunkt** — og det var det forkerte. Målt med en rigtig `runPass` og den rigtige `sendWebhook` mod en lokal endpoint, nul kode ændret: to sites i ét pass (ét svarer med det samme, ét timeout`er 800 ms senere) gav `timestamp …11.849Z` og `…13.870Z`, mens **begge checks startede** `…11.042Z`. `timestamp` er hvornår POST-kroppen blev bygget — efter `printPass` og efter modtagerens egen svartid — så en kanal der viser feltet som "sitet brød kl. 14:26" læser en leveringstid som en måling, og den rigtige måling var **ikke i payloaden**. Værre: samme payload **påstod en overgang DeskUptime aldrig så**. En state-fil med et pass fra 41 dage siden (loopet dødt, sitet aldrig nede) gav `{"type":"up","message":"is UP (200) — 12ms"}` — `type: "up"` er en maskinlæsbar påstand om en tilstandsendring, målt mod en læsning fra seks uger siden. `deskuptime status` sagde på samme fil `stale — last check 41 d ago`, og kundenapporten sagde det samme: de to menneske-flader havde alderen, payloaden havde intet. Rettelsen følger rækken: `readEvent()` i `src/status.js` er den ene ejer af `TRANSITION` og af den ene sætning; `runPass` registrerer kun de to rå fakta (`measuredAt` = passens egen tid, `previousChecked` = tidspunktet for den måling, overgangen sammenlignes med) og **spørger** i den gren, der kender sin egen type; `sendWebhook` spørger om samme hændelse med **passens tid som reference**, så `message` og `transition` ikke kan modsige hinanden. `observed` kræver et forudgående pass der både **er til stede** og er **friskt** — den første regel genbrugte `isCheckStale`, som med vilje siger at en *fraværende* tid ikke er stale (korrekt for rapporten, der allerede siger "not checked yet"), så den påstod en overgang mod ingenting; målingen ovenfor fangede det, og hullet har nu sin egen test. 6 nye tests + 4 målte mutationer (3 / 2 / 3 / 1 fejl) → **279/279**; `npm run audit` 0/0; `node --check`, `matrix --check`, `sh -n`/`bash -n`, YAML- og diff-check grønne på Node 26.7.0. **M4 var kun fanget strukturelt** — en adfærdstest kan stadig ikke se en duplikeret ejer, så låsen tæller `= readEvent(`-kaldene (præcis 3) og forbyder `observed`/`unobserved`-strenge i `watch.js`. **Ingen afhængighed ændret, ingen ny claim, matrixen urørt, ingen exit-kode ændret, `timestamp`s betydning uændret (kun additive felter), ingen deploy-note nødvendig.** Næste: ❓ 2/❓ 3 hvis de er besvaret; ellers P1-18 — samme måling på den eneste overflad tilbage, der ikke er målt på de samme tal-spørgsmål: `deskuptime status` sin **license-flade** (`src/license.js` + `cli.js status`).
...
### P1-18 — NÆSTE — Mål license-fladen i `deskuptime status` på de samme tal-spørgsmål (`ceo/status-license-truth`)
...
**Hvad målingen skal spørge (kun måling først, ingen kode før den har svaret):**

1. Kan `status` skrive et tal eller en dato uden at have målt den? Kandidater: `days` til udløb, `plan`-åretstal, antal dage siden validering, `devices_in_use`.
2. Modsiger license-fladen sig selv — to tal i samme blok, der ikke kan begge være rigtige (P1-13's fund i resumelinjen)?
3. Skelner den mellem de fire synlige tilstande (`active`, `cached/offline`, `unverified`, `invalid`, `free`) på samme måde som `docs/license-lifecycle.md` beskriver, eller er et af ordene tavst?
4. Er `deskuptime status` stadig read-only og netværksfri efter alt dette? (Det er en hærdet egenskab, ikke en antaget.)

**Acceptkriterier (skal formuleres efter målingen, som i de øvrige opgaver):**

1. Målingen er skrevet ned i denne plan **før** der ændres kode, med de konkrete tal den fandt.
2. Ét fund rettes med **én** ejer i `src/status.js` eller `src/license.js`, og de øvrige flader spørger i stedet for at eje.
...
**Filer:** `src/license.js`, `src/cli.js` (`status`-grenen), `src/status.js`, `test/statusline.test.js` eller `test/license.test.js`, `docs/license-lifecycle.md`.
